...
func TestMinimize(t *testing.T) {
	...
	tests := []struct {
		...
		// Predicate always returns false, so must get the same program.
		{
			...
			2,
			func(p *Prog, callIndex int, _ int) bool {
				if len(p.Calls) == 0 {
		...
		},
		{
			...
			"minimize$0(0x1, 0x1)\n",
			-1,
			func(p *Prog, callIndex int, _ int) bool { return len(p.Calls) == 1 },
...
func TestMinimizeRandom(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		for _, crash := range []bool{false, true} {
			...
			copyP := p.Clone()
			minP, _ := Minimize(p, len(p.Calls)-1, crash, func(p1 *Prog, callIndex int, _ int) bool {
				if r.Intn(2) == 0 {
					return false
				}
				copyP = p1.Clone()
...
func TestMinimizeCallIndex(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		...
		p1, ci1 := Minimize(p, ci, r.Intn(2) == 0, func(p1 *Prog, callIndex int, _ int) bool {
			return r.Intn(2) == 0
		})
