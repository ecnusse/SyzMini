...
var (
	...
	flagInfluence_write = flag.String("influence_write", "", "read influence data into this file periodically")
)
...
func (mgr *Manager) initBench() {
	...
	go func() {
		for {
			...
			// consume code
			influence_count := 0
			for i := 0; i < len(mgr.InfluenceMatrix); i++ {
				for j := 0; j < len(mgr.InfluenceMatrix); j++ {
					if mgr.InfluenceMatrix[i][j] == 1 {
						influence_count++
					}
				}
			}
			if *flagInfluence_write != "" && mgr.InfluenceMatrix != nil {
				saveArrayToFile(*flagInfluence_write, mgr.InfluenceMatrix)
			}
			...
			// consume code
			vals["influence"] = uint64(influence_count)
			mgr.mu.Unlock()
...
// consume code
func saveArrayToFile(filename string, data interface{}) error {
...
func (mgr *Manager) updateInfluence(InfluenceMatrix [][]uint8) {
	if mgr.InfluenceMatrix == nil {
		mgr.InfluenceMatrix = InfluenceMatrix
	} else { // increased update influence
		for i := 0; i < len(InfluenceMatrix); i++ {
			for j := 0; j < len(InfluenceMatrix); j++ {
				if InfluenceMatrix[i][j] == 1 {
					mgr.InfluenceMatrix[i][j] = 1
					// fmt.Printf("manager:receive InfluenceMatrix\n")
				}
			}
		}
	}
